...
@section('head_title', str_replace('-',' ',$store['store_name']).' | '.getcong('site_name') )
...
<div class="container entry-content">
    ...
    <div class="breadcrumb">
        ...
        <span><a href="/stores">Stores</a></span>
        <i class="fa fa-angle-right"></i> 
        <span>{{$store['store_name']}}</span>
    </div> 
	<section class="freechrgcoupn-voters well well-sm clearfix">
		<div class="col-sm-9">
			<h3>{{$store['store_name']}} ({{$ListingsCount}})</h3>
			<ul class="list-inline">
			@if(isset(Auth::user()->usertype))
				<li id="fav_{{$store_slug}}" class="favrt-btn">
					@if(\App\ListingsVote::where(array('lmd_id'=>'fav','store'=>$store_slug,'type'=>'Favourite','UserId'=>$UserId))->count()=="0")
					<a class="btn btn-default" href="javascript:;" onclick="favourite('{{$store_slug}}','Favourite','add')"><i class="fa fa-heart-o"></i> Favourite</a>
				@else
					<a class="btn btn-default" href="javascript:;" onclick="favourite('{{$store_slug}}','Favourite','remove')"><i class="fa fa-heart"></i> Favourite</a>
				@endif
				</li>
				@else
					<li id="fav_{{$store_slug}}" class="favrt-btn">
				...
				<li class="stars " id="staticstar">
					<?php $rt=0; $totalrow=0; ?>
					@foreach(\App\Ratings::where('store',$store_slug)->get() as $reviews)
				...
				<li class="visiting">Visit <a href="{{$listings[0]['smartLink']}}" target="_blank">{{$store['store_name']}}</a></li>
				<input  type="hidden" id="storewe" name="store" value="{{$store_slug}}" />
				<input  type="hidden" id="categorywe" name="category" value="nothing" />
		...
		<div class="col-sm-3 panel">
			<img class="img-responsive" src="{{$store['store_logo']}}" class="img-responsive" />
		</div>
	</section>
					...
					<div class="item list-group-item changing">
						<div class="border clearfix">
							<div class="cashbackcoupn">
								...
								<!--p class="bgg-red">{{$listing->discount}}</p-->
							</div>
...
{{$listing->minimum_transaction}} </span>,<?php }else{ echo "Maximum Cashback: None";}?>
			...
			<!-- /. main content -->
			<!-- sidebar -->
			@include('includes.sidebar_store')
			<!-- /.sidebar -->
		</div>
	</section>
